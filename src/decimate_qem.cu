// CUDA QEM edge-collapse prototype adapted from the validated CuMesh port in
// ref/trellis.cpp/src/decimate_qem.cu (pwilkin/trellis.cpp 2516c48).
//
// This file is compiled only for PIXAL3D_ENABLE_CUDA builds. It keeps the
// topology and threshold-ladder semantics of the CPU implementation while
// moving adjacency construction, edge deduplication, cost evaluation,
// independent-set selection, and compaction to CUDA. The caller must keep the
// CPU implementation as an explicit fallback because GPU and CPU tie ordering
...
__global__ void k_propagate(const uint64_t* edges, const int* v2f, const int* off,
                            ...
                            unsigned long long* vertex_prop) {
    int t = blockIdx.x*blockDim.x + threadIdx.x; if (t >= E) return;
    uint64_t e = edges[t]; int e0 = (int)(e >> 32), e1 = (int)(e & 0xffffffffu);
    unsigned long long p = ((unsigned long long)__float_as_uint(cost[t]) << 32) | (unsigned int)t;
    // Face ownership prevents concurrent collapses from sharing a face. Vertex
    // ownership is also required: two edges can share a high-valence vertex
    // without sharing a face, and would otherwise race while writing V[e0].
    atomicMin(&vertex_prop[e0], p);
    atomicMin(&vertex_prop[e1], p);
...
__global__ void k_collapse(float3* V, int3* faces, const uint64_t* edges, const int* v2f, const int* off,
                           const float* cost, const unsigned long long* prop,
                           const unsigned long long* vertex_prop, const uint8_t* boundary,
    ...
    if (vertex_prop[e0] != p || vertex_prop[e1] != p) return;
...
static bool simplify_round_gpu(float3*& d_verts, int& V, int3*& d_faces, int& F,
    ...
    if (E > 0) {
        ...
        unsigned long long* d_vertex_prop = nullptr;
        GPU_OK(gpuMalloc(&d_prop, sizeof(unsigned long long) * (size_t)F));
        GPU_OK(gpuMalloc(&d_vertex_prop, sizeof(unsigned long long) * (size_t)V));
        GPU_OK(gpuMemsetAsync(d_prop, 0xff, sizeof(unsigned long long) * (size_t)F, s)); // UINT64_MAX
        GPU_OK(gpuMemsetAsync(d_vertex_prop, 0xff, sizeof(unsigned long long) * (size_t)V, s)); // UINT64_MAX
        k_propagate<<<NB(E), BLK, 0, s>>>(d_edges, d_v2f, d_off, d_cost, E, d_prop, d_vertex_prop);
        GPU_LAST();
        ...
        k_collapse<<<NB(E), BLK, 0, s>>>(d_verts, d_faces, d_edges, d_v2f, d_off, d_cost, d_prop,
                                         d_vertex_prop, d_boundary, E, thresh, d_vdead, d_fdead);
        GPU_LAST();
        ...
        gpuFree(d_boundary); gpuFree(d_qem); gpuFree(d_cost); gpuFree(d_prop); gpuFree(d_vertex_prop);
